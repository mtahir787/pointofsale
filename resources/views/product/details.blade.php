            ...
            <tbody>
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                        {{ $productitemdetail['id'] }}
                    </th>
                    <td class="px-6 py-4">
                        {{ $productitemdetail['product_name'] }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $productitemdetail['product_description'] }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $productitemdetail['product_quantity'] }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $productitemdetail['sale_price'] }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $productitemdetail['purchase_price'] }}
                    </td>
                    <td class="px-6 py-4">
                        {!! DNS1D::getBarcodeHTML($productitemdetail['product_barcode'], 'UPCA') !!}
                    </td>
                    @foreach ($catagories as $category)
                        @if ($productitemdetail['category_fid'] === $category['id'])
                            <td class="px-6 py-4">
                ...
                @endforeach
                <td class="px-1 py-4">
                    <a href={{ '/product/destroy/' . $productitemdetail['id'] }}
