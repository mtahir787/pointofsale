        ...
        <div id="alert-3" class="flex p-4 mb-4 bg-green-100 rounded-lg dark:bg-green-200" role="alert">
            ...
            <div class="ml-3 text-sm font-medium text-green-700 dark:text-green-800">
                @if (@session('message') === 'created')
                    The Product has Created successfully!
    ...
    @if (@session('message') === 'deleted')
        <div id="alert-3" class="flex p-4 mb-4 bg-red-100 rounded-lg dark:bg-red-200" role="alert">
            ...
            <div class="ml-3 text-sm font-medium text-red-700 dark:text-red-800">
                The Product has Deleted successfully!
            </div>
            {{-- <button type="button"
                class="ml-auto -mx-1.5 -my-1.5 bg-green-100 text-green-500 rounded-lg focus:ring-2 focus:ring-green-400 p-1.5 hover:bg-green-200 inline-flex h-8 w-8 dark:bg-green-200 dark:text-green-600 dark:hover:bg-green-300"
                data-dismiss-target="#alert-3" aria-label="Close">
            ...
            <tbody>
                @foreach ($products as $product)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4">
                            {!! DNS1D::getBarcodeHTML($product['product_barcode'], 'UPCA') !!}
                        </td>
                        @foreach ($categories as $category)
                            @if ($product['category_fid'] === $category['id'])
                                <td class="px-6 py-4">
                        ...
                        <td class="px-1 py-4 grid grid-cols-3">
                            ...
                            <a href={{ '/product/trash/' . $product['id'] }}
